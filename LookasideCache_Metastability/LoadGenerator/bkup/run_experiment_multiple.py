...
for iter in range(0, len(lambda_trigger_arr)):
    ...
    result_file_name = "results_warm_cache/result_" + str(float(_lambda)) + "_" + str(float(zipf_parameter)) + "_DUR_" + str(duration_of_test) + "_TRSZ_" + str(trigger_size) "_TMOUT_"+ timeout +".txt"
   ...
   try:
        ...
        connPath = '/home/ubuntu/Cache_Experiments' + result_file_name 
        connPrivateKey = './cache_workers.pem'

        # Use scp to send file from local to host.
        scp = subprocess.Popen(['scp', '-i', connPrivateKey, result_file_name, '{}@{}:{}'.format(connUser, connHost, connPath)])
